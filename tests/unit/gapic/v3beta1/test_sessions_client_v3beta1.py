...
class TestSessionsClient(object):
    ...
    def test_match_intent(self):
        ...
        expected_response = session_pb2.MatchIntentResponse(**expected_response)
        ...
        assert len(channel.requests) == 1
        expected_request = session_pb2.MatchIntentRequest(
            session=session, query_input=query_input
        )
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request

    def test_match_intent_exception(self):
        # Mock the API response
        channel = ChannelStub(responses=[CustomException()])
        patch = mock.patch("google.api_core.grpc_helpers.create_channel")
        with patch as create_channel:
    ...
    def test_fulfill_intent(self):
        # Setup Expected Response
        response_id = "responseId1847552473"
        output_audio = b"24"
        expected_response = {"response_id": response_id, "output_audio": output_audio}
        expected_response = session_pb2.FulfillIntentResponse(**expected_response)
        ...
        assert len(channel.requests) == 1
        expected_request = session_pb2.FulfillIntentRequest()
        actual_request = channel.requests[0][1]
        assert expected_request == actual_request

    def test_fulfill_intent_exception(self):
        # Mock the API response
        channel = ChannelStub(responses=[CustomException()])
        patch = mock.patch("google.api_core.grpc_helpers.create_channel")
        with patch as create_channel:
